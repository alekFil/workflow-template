...
- `.gitignore` — добавлены `.claude/settings.local.json`, `.claude/chat_history/`, `.env` (ADR-009)
...
- `.context/discussions/` — два обсуждения: curl-установка и mini-версия
- `.context/notes/` — два патч-файла для переноса улучшений в другие проекты
...
- **template-mini** (Приоритет 1): драфт готов в `.context/discussions/`, реализация не начата
...
- `ADR-009` реализован, изменения не зафиксированы в git — нужен `фиксируем`
- `.claude/settings.local.json` — добавлен в `.gitignore`; файл остаётся локально, не мешает
- `template-mini` — следующий логичный шаг; нужно решение брать ли в работу
- `.context/notes/` — назначение нигде не зафиксировано (патч-файлы для переноса в другие проекты)
