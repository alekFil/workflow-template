...
### 3.3 Скрипт инициализации

`scripts/init-project.sh` — разворачивает шаблон в новый проект:
...
5. Привязывает новый remote (или создаёт репо на GitHub)
...
- Скрипт инициализации (заглушка — логика реализуется отдельной задачей)
